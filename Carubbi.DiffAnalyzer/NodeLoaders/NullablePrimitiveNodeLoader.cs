...
namespace Carubbi.DiffAnalyzer.NodeLoaders
{
    public class NullablePrimitiveNodeLoader : NodeLoaderBase
    {
        ...
        public override void LoadNode(string propertyName, object oldInstance, object newInstance, List<DiffComparison> comparisons, int depth)
        {
            ...
            if (oldInstance != null && (string) oldInstance != string.Empty && !oldInstance.GetType().IsPrimitive)
            {
                var oldHasValue = Convert.ToBoolean(newInstance.GetType().InvokeMember(HAS_VALUE_PROPERTY, BindingFlags.GetProperty, null, oldInstance, null));
            ...
            if (newInstance != null && (string) newInstance != string.Empty && !newInstance.GetType().IsPrimitive)
            {
                var newHasValue = Convert.ToBoolean(newInstance.GetType().InvokeMember(HAS_VALUE_PROPERTY, BindingFlags.GetProperty, null, newInstance, null));
